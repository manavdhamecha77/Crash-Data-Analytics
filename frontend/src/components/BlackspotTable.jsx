...
const BlackspotTable = () => {
  ...
  // DBSCAN Parameters
  const [eps, setEps] = useState(500); // 500 meters
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="card" style={{ height: '500px', padding: 0, overflow: 'hidden', position: 'relative' }}>
        <div style={{ position: 'absolute', top: '10px', right: '10px', zIndex: 1000, background: 'rgba(255,255,255,0.9)', padding: '8px', borderRadius: '4px', border: '1px solid var(--rule)', fontSize: '12px' }}>
          <strong>Map Layers:</strong> Use control to toggle Before/After
        </div>
        <MapContainer center={center} zoom={7} style={{ height: '100%', width: '100%' }}>
          ...
          <LayersControl position="topright">
            <LayersControl.Overlay checked name="Before: Raw Points">
              <LayerGroup>
                {points.map((point, idx) => (
                  <CircleMarker
                    ...
                    radius={2}
                    pathOptions={{ color: 'var(--ink-3)', fillColor: 'var(--ink-3)', fillOpacity: 0.4, weight: 0 }}
                  />
                ))}
              </LayerGroup>
            </LayersControl.Overlay>

            <LayersControl.Overlay checked name="After: Cluster Formation">
              <LayerGroup>
                {clusters.map((spot) => (
                  <div key={`cluster-group-${spot.id}`}>
                    <Circle 
                    ...
                    >
                      <Popup>
                        <div style={{ padding: '4px' }}>
                          ...
                          <p style={{ fontSize: '12px' }}>Accidents: {spot.accidents}</p>
